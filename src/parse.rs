...
///
/// Only `EndOfStream` errors are handled at runtime. All other errors
...
impl Parse {
    ...
    /// Return the next entry. Array frames are arrays of frames,
    /// so the next entry is a frame.
    ...
    ///
    /// If the next entry can't be represented as a String, then an error is returned.
    pub(crate) fn next_string(&mut self) -> Result<String, ParseError> {
        match self.next()? {
            // Both `Simple` and `Bulk` representation may be strings.
            // Strings are parsed to UTF-8.
            //
    ...
    ///
    /// If the next entry can't be represented as raw bytes, an error is returned.
    pub(crate) fn next_bytes(&mut self) -> Result<Bytes, ParseError> {
        match self.next()? {
            ...
            // raw bytes, they are considered separated types.
    ...
    ///
    /// This includes `Simple`, `Bulk`, and `Integer` frame bytes. `Simple` and `Bulk`
    /// frame types are parsed.
    ///
    /// If the next entry can't represented as an integer, then an error is returned.
