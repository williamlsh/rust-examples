use crate::{
    cmd::{Command, Unknown},
    Connection, Db, Frame, Parse, ParseError, Shutdown,
};
use bytes::Bytes;
use std::pin::Pin;
...
///
/// Once the client enters the subscribe state, it's not supposed to issue any
...
///
/// When no channel is specified, the client is unsubscribed from all
/// the previously subscribed channels.
#[derive(Clone, Debug)]
...
/// `broadcast::Receiver`. We use `stream!` to create a `Stream`
/// that consumes messages. Because `stream!` values can't be named,
/// we box the stream using a trait object.
...
impl Subscribe {
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> crate::Result<Subscribe> {
        ...
        loop {
            match parse.next_string() {
                // A string has been consumed from `parse`, push it into
    ...
    ///
    /// This function is the entry point and includes the initial ist of
    ...
    pub(crate) async fn apply(
        ...
        // Messages are then fanned out to all clients currently subscribed to the channels.
        //
        // An individual client may subscribe to multiple channels and may dynamically add
        // and remove channels from its subscription set. To handle this, a `StreamMap` is
        // used to tack active subscriptions. The `StreamMap` merges messages from individual
...
/// Create the response to a subscribe request.
///
...
impl Unsubscribe {
    /// Create a `Unsubscribe` command with the given `channels`.
